...
# Upload Dataset
st.sidebar.header("/content/diabetes (3).csv") # changed header text for clarity
uploaded_file = st.sidebar.file_uploader("Choose a CSV file", type=["csv"]) # changed to use file_uploader correctly
if uploaded_file is not None:
    ...
    if len(selected_features) > 1:
        fig = sns.pairplot(diabetes_data[selected_features]) # store pairplot in fig
...
# Upload Dataset
st.sidebar.header("/content/diabetes (3).csv")
uploaded_file = st.sidebar.file_uploader("/content/diabetes (3).csv", type=["csv"])
if uploaded_file is not None:
    diabetes_data = pd.read_csv(uploaded_file)
    st.sidebar.success("File Uploaded Successfully!")
